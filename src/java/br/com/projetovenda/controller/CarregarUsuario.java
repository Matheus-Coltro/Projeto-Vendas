...
public class CarregarUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            Integer idu = Integer.parseInt(
                    request.getParameter("idu"));
            GenericDAO dao = new UsuarioDAO();
            Usuario usuario = new Usuario();
            
           usuario = (Usuario) dao.carregar(idu);

            request.setAttribute("usuario", usuario);
            request.getRequestDispatcher(
                     "cadastrar-usuario.jsp").forward(request, response);
            
        }catch(Exception e){
            System.out.println(
                    "Erro ao alterar usuarioCTR " + e.getMessage());
        }
    }
